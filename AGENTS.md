...
- Use the comms log for reliability between agents: `python ~/dev/nudge/swarm/cli.py send <cfg> <pane> "msg"` or `log_broadcast`.
- Inspect: `python ~/dev/nudge/swarm/cli.py log <cfg> [--pending] [--pane X.Y]`, `python ~/dev/nudge/swarm/cli.py cursors <cfg>`.
- Direct/manual still works: `~/dev/nudge/tmux-send <target> "message"`.

Do NOT use raw `tmux send-keys ... Enter` (fails to submit reliably).

Swarm scripts: `~/dev/nudge/swarm`.
