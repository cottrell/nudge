...
- Querying a local database (`agent-monitor`'s `sessions.db`) works for local token spend, but cannot fetch remote-side subscription limits (like remaining Claude Pro window hours).
