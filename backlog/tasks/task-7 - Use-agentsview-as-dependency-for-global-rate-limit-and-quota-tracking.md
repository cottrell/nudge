---
id: TASK-7
title: Use agent-monitor as dependency for global rate limit and quota tracking
status: To Do
assignee: []
...
Transition from fragile terminal screen-scraping (`monitor.c`) to structured, global token/rate-limit tracking using `agent-monitor` as a dependency. 
...
### Approach
1. **Leverage `agent-monitor` Syncing:** Use `agent-monitor` to watch the filesystem (via `fsnotify` / `inotify`) and automatically ingest all agent transcripts/SQLite files into `~/.agent-monitor/sessions.db`.
