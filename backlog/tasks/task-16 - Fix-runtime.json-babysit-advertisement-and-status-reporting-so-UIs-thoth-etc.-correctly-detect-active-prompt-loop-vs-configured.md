...
id: TASK-16
title: >-
  Fix runtime.json 'babysit' advertisement and status reporting so UIs (dashboard
...
Observed: For demo-data (session 'cd'), /tmp/nudge-swarm/cd/runtime.json advertises 'babysit' entries with has_long_prompt:true for panes even when no pids, no specs, no state files, and `aiswarm status swarm/cd.yaml` reports the session as 'missing' (or babysit not active).
...
- Investigate/update any dashboard or demo-data code that parses this (but primary fix in nudge reporting).
...
- [ ] #3 `aiswarm status` and runtime.json agree on babysit state for demo-data/cd and similar swarms
- [ ] #4 External consumers like dashboard no longer falsely report babysit 'running' when cli status shows off/stopped/not-started/missing
...
---
Runtime fix landed in same commit. Should help dashboard/demo-data false positives. Awaiting user testing and pane reviews.
---
<!-- COMMENTS:END -->
