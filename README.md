...
- `babysit.sh <session>` — poll state, nudge on idle, wait on unknown/rate_limited, with an optional max continuous non-idle backstop
...
```bash
python swarm/cli.py init my_swarm --root /path/to/project -D
...
The swarm interface now lives under one CLI:
- `swarm/cli.py apply` reconciles tmux topology, monitors, and initial pane commands
